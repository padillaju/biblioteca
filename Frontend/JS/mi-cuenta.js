...
function logout() {
  // Confirmación más visible y segura
  const confirmMessage = "⚠️ ¿ESTÁS SEGURO DE QUE QUIERES CERRAR SESIÓN?\n\n" +
                        "Esta acción cerrará tu sesión de forma permanente.\n" +
  ...
  function clearAllClientData() {
    try {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && (key.includes('user') || key.includes('session') || key.includes('cart') || key.includes('admin'))) {
    ...
    if (data.success) {
      ...
      .then(verifyData => {
        if (!verifyData.isAuthenticated) {
          showNotification("✅ Sesión cerrada de forma segura", "success");
